...
def similarity_or_distance_graph(matrix,graph_labels,fake):

    # if (fake != 1) or (fake != 0):
    #     raise Exception("fake must be 0 or 1")
    ...
    if fake==1:# matrix contains fake
        news = list(graph_df.index[:len(graph_df) - 1])
        # For labels
        news.append("fake")

        if graph_labels == "all":
            plt.scatter(graph_df.iloc[:, 0], graph_df.iloc[:, 1], s=50, cmap='viridis')
            ...
            plt.scatter(graph_df.iloc[:, 0], graph_df.iloc[:, 1], s=50, cmap='viridis')
            plt.annotate("fake", (graph_df.iloc[len(graph_df) - 1, 0], graph_df.iloc[len(graph_df) - 1, 1]))
            plt.title("Visualization of data")
            plt.xlabel("PCA 1")
            plt.ylabel("PCA 2")
            plt.show()
    else:# matrix does not contain fake
        # For labels
